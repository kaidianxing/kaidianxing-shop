...
/**
 * 头条渠道处理器
 * Class ToutiaoHandler
 * @package shopstar\components\authorization
 * @author 青岛开店星信息技术有限公司
 */
class ToutiaoHandler extends AuthorizationHandlerInterface
{
    ...
    public function auth(array $options = []): array
    {
        ...
        return ByteDanceComponent::getInstance(ByteDanceConstant::CHANNEL_TOUTIAO)->factory->auth->session($code, $anonymousCode);
    }
    ...
    public function login(string $sessionId)
    {
        ...
        try {
            // 解密
            $result = MemberToutiaoModel::getUserInfo(trim($info['encryptedData']), trim($info['session_key']), trim($info['iv']), trim($info['rawData']), trim($info['signature']));
            // 注册/更新 用户信息
            $memberInfo = MemberToutiaoModel::checkMember($result, ClientTypeConstant::CLIENT_BYTE_DANCE_TOUTIAO);
        ...
        return MemberLoginModel::login($memberInfo['id'], $sessionId, $memberInfo, ClientTypeConstant::CLIENT_BYTE_DANCE_TOUTIAO);
    }
