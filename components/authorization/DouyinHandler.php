 ...
 * @author 青岛开店星信息技术有限公司
 */
class DouyinHandler extends AuthorizationHandlerInterface
{
    ...
    public function auth(array $options = []): array
    {
        ...
        return ByteDanceComponent::getInstance(ByteDanceConstant::CHANNEL_DOUYIN)->factory->auth->session($code, $anonymousCode);
    }
    ...
    public function login(string $sessionId)
    {
        ...
        try {
            // 解密
            $result = MemberDouyinModel::getUserInfo(trim($info['encryptedData']), trim($info['session_key']), trim($info['iv']), trim($info['rawData']), trim($info['signature']));
            // 注册/更新 用户信息
            $memberInfo = MemberDouyinModel::checkMember($result, ClientTypeConstant::CLIENT_BYTE_DANCE_DOUYIN);
        ...
        return MemberLoginModel::login($memberInfo['id'], $sessionId, $memberInfo, ClientTypeConstant::CLIENT_BYTE_DANCE_DOUYIN);
    }
